...
from combatgame.game_logic.skills import Skills, BaseSkill, SkillEffects
...
class BaseCharacter:
    ...
    def defend(self) -> None:
        """defend method.
        raises defense points by 1 to 10.

        Returns
        -------
        log : str
            The log for using defend.
        """
        # reduce speed points by 1
        self.speed_points -= 1

        # defense points increase
        dp_increase = random.randint(1, 10)

        # raise defense points by 1 to 10
        self.defense_points += dp_increase

        # cap the increase to be the max value of defense points
        self.defense_points = min(self.defense_points, self.max_defense_points)

        # different log if defense points is maxed
        if self.defense_points == self.max_defense_points:
            return f"{self.name} used defend and maxed out their defense!"

        return f"{self.name} used defend and gained {dp_increase}DP."
    ...
    def is_player(self):
        """Returns whether instance is a player character.
        
        Returns
        -------
        bool : True if instance is player, False otherwise.
        """
        return True
